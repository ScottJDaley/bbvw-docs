...
def get_move_display(m_name, move_data, move_stat_changes, base_path="../", show_tm=False):
    ...
    if show_tm:
        ...
        return f"| {tm_num} | [{display_name}]({base_path}moves/{m_info['name']}.md) | {type_icon} | {cat_icon} | {power} | {acc} | {m_info['pp']} |"
    return f"| [{display_name}]({base_path}moves/{m_info['name']}.md) | {type_icon} | {cat_icon} | {power} | {acc} | {m_info['pp']} |"
...
def generate_pokemon_page(name, base_data, rom_data, move_data, ability_data, locations):
    ...
    md = f"# {name.capitalize()}\n\n![{name}](../img/pokemon/{p_base['id']:03}.png)\n\n"
    
    curr_types = p_rom.get('types') or p_base['types']
    md += "## Type\n"
    if p_rom.get('types') and p_rom['types'] != p_base['types']:
        md += "Original: " + ' '.join([f'![{t}](../img/types/{t}.png)' for t in p_base['types']]) + "  \nNew: " + ' '.join([f'![{t}](../img/types/{t}.png)' for t in p_rom['types']]) + "\n\n"
    else: md += ' '.join([f'![{t}](../img/types/{t}.png)' for t in p_base['types']]) + "\n\n"
        
    md += "## Evolution\n"
    paths = get_full_evolution_chains(p_base); max_stages = max(len(p) for p in paths)
    md += "|" + " | ".join(["Stage" if i%2==0 else "" for i in range(2*max_stages-1)]) + " |\n"
    md += "|" + " | ".join([":---:" for _ in range(2*max_stages-1)]) + " |\n"
    for path in paths:
        row_parts = []
        for i in range(max_stages):
            if i < len(path):
                step = path[i]; p_norm = normalize_name(step['name']); p_info = base_data.get(p_norm)
                ...
                if i < max_stages - 1:
                    if i + 1 < len(path):
                        next_step = path[i+1]; next_p_norm = normalize_name(next_step['name']); rom_method = ""
                        ...
                        if current_p_rom.get('evolution'):
                            for rev in current_p_rom['evolution']:
                                if rev['target'] and normalize_name(rev['target']) == next_p_norm: rom_method = rev['method']
    ...
    def get_ab_info(ab_name):
        norm = normalize_name(ab_name); desc = ability_data.get(norm, {}).get('description', '')
        return f"**[{ab_name}](../abilities/{norm}.md)**: {desc}"
    md += "| Slot | Original | New |\n| --- | --- | --- |\n"
    orig1 = orig_abs[0] if len(orig_abs) > 0 else "-"; new1 = ab1_new if ab1_new else orig1
    md += f"| Ability 1 | {get_ab_info(orig1) if orig1 != '-' else '-'} | {get_ab_info(new1)} |\n"
    orig2 = orig_abs[1] if len(orig_abs) > 1 else "-"; new2 = ab2_new if ab2_new else (orig2 if orig2 != "-" else "-")
    ...
    for stat in ['hp', 'attack', 'defense', 'special-attack', 'special-defense', 'speed']:
        orig_val = p_base['stats'].get(stat, 0); new_data = p_rom.get('stats', {}).get(stat.replace('-', '_'))
        new_val = new_data['new'] if new_data else orig_val; total_val += new_val
    ...
    orig_total = sum(p_base['stats'].values()); total_new_data = p_rom.get('stats', {}).get('total'); total_new = total_new_data['new'] if total_new_data else total_val
...
def generate_move_page(name, move_info, pokemon_list, m_rom, base_data):
    ...
    for p in sorted(pokemon_list):
        p_info = base_data.get(normalize_name(p))
        sprite = f'![{p}](../img/pokemon/{p_info["id"]:03}.png)' if p_info else ""
        md += f"| {sprite} | [{p.capitalize()}](../pokemon/{normalize_name(p)}.md) |\n"
    return md

def generate_ability_page(name, ability_info, pokemon_list, base_data):
    md = f"# {name.replace('-', ' ').capitalize()}\n\n"
    ...
    if route_data:
        for sec in route_data['sections']:
            ...
            for enc in sec['encounters']:
                m = enc['method']; methods[m] = methods.get(m, []) + [enc]
            for m, encs in methods.items():
                m_lower = m.lower().replace(',', ''); icon = "grass-normal.png"
                if 'surf special' in m_lower: icon = "surf-special.png"
                elif 'fish special' in m_lower: icon = "fishing-special.png"
                elif 'cave special' in m_lower: icon = "cave-special.png"
                elif 'special' in m_lower: icon = "grass-special.png"
                ...
                for enc in encs:
                    p_norm = normalize_name(enc['pokemon']); p_info = base_data.get(p_norm)
...
if __name__ == "__main__":
    ...
    for p in pkmn_list:
        p_name = p['name']; p_rom = romhack['pokemon_changes'].get(p_name, {})
        p_abs = [p_rom.get('abilities', {}).get('one'), p_rom.get('abilities', {}).get('two')]
        p_abs = [extract_specific_ability(a, p_name) for a in p_abs if a]
        if not any(p_abs): p_abs = p['abilities']
        for ab in p_abs:
            ab_norm = normalize_name(ab)
            if ab_norm not in final_ability_to_pokemon: final_ability_to_pokemon[ab_norm] = []
            final_ability_to_pokemon[ab_norm].append(p_name)
        
        p_moves = set([m['name'] for m in p['moves']])
        p_moves_rom = romhack['move_changes'].get(p_name, [])
        for rm in p_moves_rom: p_moves.add(rm['name'])
        for tmline in p_rom.get('tm_hm', []):
            matches = re.findall(r'(TM|HM)\d+\s*,?\s*([^,.\n]+)', tmline)
            for _, mname in matches: p_moves.add(normalize_name(mname))
        for tline in p_rom.get('tutor', []):
            matches = re.findall(r'([^,.\n]+)', tline)
            for mname in matches: p_moves.add(normalize_name(mname))
        for mname in p_moves:
            if mname not in final_move_to_pokemon: final_move_to_pokemon[mname] = []
            final_move_to_pokemon[mname].append(p_name)
    ...
    with open("docs/pokemon/index.md", 'w') as f:
        ...
        for g_name, start, end in gens:
            ...
            for p in pkmn_list:
                if start <= p['id'] <= end:
                    f.write(f"| {p['id']:03} | ![{p['name']}](../img/pokemon/{p['id']:03}.png) | [{p['name'].capitalize()}]({normalize_name(p['name'])}.md) |\n")
            f.write("\n")
    ...
    for a_name, a_info in sorted(base['abilities'].items()):
        md = generate_ability_page(a_name, a_info, final_ability_to_pokemon.get(a_name, []), base['pokemon'])
    ...
    all_locations = romhack['trainer_order']
    ...
    for line in lines:
        if line.startswith('nav:'):
            in_nav = True; new_lines.append('nav:\n  - Home: README.md\n  - Pokemon:\n    - pokemon/index.md\n')
            for g_name, start, end in gens:
